...
variable "family" {
  type    = string
  default = "aspect-workflows-ubuntu-2604-kitchen-sink"
}
...
# Canonical, Ubuntu, 26.04 LTS, <arch> resolute image build on <rev>
data "amazon-ami" "ubuntu" {
  filters = {
    virtualization-type = "hvm"
    name                = "ubuntu/images/hvm-ssd-gp3/ubuntu-resolute-26.04-${var.arch}-server-20260904"
...
build {
  ...
  provisioner "shell" {
    inline = concat([
      ...
      # Disable Ubuntu 26.04 AppArmor mount operations restrictions so Bazel can use linux-sandbox
